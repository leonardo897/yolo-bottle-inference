...
def get_latest_frame(cap):
    """Descarta frames acumulados, retorna o mais recente."""
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        # Verifica se ainda há frame na fila do buffer
        if not cap.grab():  # grab é mais leve, não decodifica
            break
    return frame  # este é o frame mais recente
